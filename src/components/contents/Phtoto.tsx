...
const Photo = ({ theme }: PhotoProps) => {
  ...
  return (
    <div className={`w-full max-w-4xl mx-auto mt-6 ${isDark ? 'bg-gray-900' : 'bg-gray-50'} rounded-xl`}>
      <div 
      ...
      >
        ...
        <div 
          ...
          style={{ transform: `translateX(-${currentIndex * (100 / imagesPerView + 4/3)}%)` }}
        >
          {images.map((image, index) => (
            <div
              key={index}
              className="flex-shrink-0 relative group"
              style={{ width: `calc(${100/imagesPerView}% - ${16 * (imagesPerView - 1) / imagesPerView}px)` }}
            >
          ...
          <>
            <button
              ...
              className={`absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/20 backdrop-blur-sm hover:bg-white/30 text-white p-3 rounded-full transition-all duration-300 hover:scale-110 z-10 ${
                currentIndex === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-lg'
              }`}
            >
              <ChevronLeft size={24} />
            </button>
